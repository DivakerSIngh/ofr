        ...
        * */
        var initColHeads = function () {
            ...
            for (var i = 0, curColHead = ''; i <= sheetOption.data.dimensions[1]; ++i) {
                if (i === 0) {
                    curColHead = '<td class="TimeSheetBid-head"><a onclick="GetRowsData();">' + sheetOption.data.sheetHead.name + '</a></td>';
                } else {
                    curColHead = '<td title="' + (sheetOption.data.colHead[i - 1].title ? sheetOption.data.colHead[i - 1].title : "") + '" data-col="' + (i - 1) + '" class="TimeSheetBid-colHead ' + (i === sheetOption.data.dimensions[1] ? 'rightMost' : '') + '" >' + sheetOption.data.colHead[i - 1].name + '</td>';
                }
                colHeadHtml += curColHead;
            }
            if (sheetOption.remarks) {
                colHeadHtml += '<td class="TimeSheetBid-remarkHead">' + sheetOption.remarks.title + '</td>';
            }
            colHeadHtml += '</tr>';
        ...
        * */
        ...
        var initRows = function () {
            for (var row = 0, curRowHtml = ''; row < sheetOption.data.dimensions[0]; ++row) {
                if (sheetOption.data.rowHead[row].type == 1) {
                    roomheader = true;
                    curRowHtml = '<tr class="TimeSheetBid-row parent">'
                }
                else {
                    roomheader = false;
                ...
                for (var col = 0, curCell = ''; col <= sheetOption.data.dimensions[1]; ++col) {
                    if (col === 0) {
                        if (roomheader) {
                            curCell = '<td title="' + (sheetOption.data.rowHead[row].title ? sheetOption.data.rowHead[row].title : "") + '"class="text-left TimeSheetBid-rowHead ' + (row === sheetOption.data.dimensions[0] - 1 ? 'bottomMost ' : ' ') + '" style="' + (sheetOption.data.rowHead[row].style ? sheetOption.data.rowHead[row].style : '') + '"><input type="checkbox" class="hddesc" ctype="R" rid="' + sheetOption.data.rowHead[row].irid + '" pid="' + sheetOption.data.rowHead[row].irpid + '" style="margin-right: 7px;"/><a class="blue" style="cursor:pointer;" onclick="Toggletr(this);">' + sheetOption.data.rowHead[row].name + '</a><i class="fa fa-arrow-circle-right" aria-hidden="true" style="float: right;" ></i></td>';
                        }
                        else {

                            //check for linked rate plans
                            if (sheetOption.data.rowHead[row].ilinked > 0)
                                curCell = '<td title="' + (sheetOption.data.rowHead[row].title ? sheetOption.data.rowHead[row].title : "") + '"class="text-left TimeSheetBid-rowHead ' + (row === sheetOption.data.dimensions[0] - 1 ? 'bottomMost ' : ' ') + '" style="' + (sheetOption.data.rowHead[row].style ? sheetOption.data.rowHead[row].style : '') + '"><input type="checkbox" class="hddesc" ctype="P"  rid="' + sheetOption.data.rowHead[row].irid + '" pid="' + sheetOption.data.rowHead[row].irpid + '" style="margin-right: 7px;"/><strong class="blue">' + sheetOption.data.rowHead[row].name + '</strong>   <span class="pull-right"><img src="../images/up-arrow.png"></span></td>';
                            else
                                curCell = '<td title="' + (sheetOption.data.rowHead[row].title ? sheetOption.data.rowHead[row].title : "") + '"class="text-left TimeSheetBid-rowHead ' + (row === sheetOption.data.dimensions[0] - 1 ? 'bottomMost ' : ' ') + '" style="' + (sheetOption.data.rowHead[row].style ? sheetOption.data.rowHead[row].style : '') + '"><input type="checkbox" class="hddesc" ctype="P" rid="' + sheetOption.data.rowHead[row].irid + '" pid="' + sheetOption.data.rowHead[row].irpid + '"  style="margin-right: 7px;"/><strong class="blue">' + sheetOption.data.rowHead[row].name + '</strong></td>';
                        }
                    } else {
                        curCell = '<td class="TimeSheetBid-cell  ' + sheetOption.data.sheetData[row][col - 1].class + '" data-row="' + row + '" data-col="' + (col - 1) + '" onclick="getdata(this);"   type="' + sheetOption.data.rowHead[row].type + '" cdate="' + sheetOption.data.colHead[col - 1].cdate + '" roomid="' + sheetOption.data.sheetData[row][col - 1].rid + '" planid="' + sheetOption.data.sheetData[row][col - 1].rpid + '" style="cursor:pointer;">' + sheetOption.data.sheetData[row][col - 1].value + '</td>';
                    }
                    curRowHtml += curCell;
                }
                if (sheetOption.remarks) {
