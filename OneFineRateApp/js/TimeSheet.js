        ...
        * */
        var initYear = function () {
            ...
            for (var i = 0, curColHead = ''; i <= sheetOption.data.dimensions[1]; ++i) {
                if (i === 0) {
                    curColHead = '<td class="TimeSheet-head"></td>';
                } else if (i === 1) {
        ...
        var initColHeads = function () {
            ...
            for (var i = 0, curColHead = ''; i <= sheetOption.data.dimensions[1]; ++i) {
                if (i === 0) {
                    ...
                    curColHead = '<th colspan="1" style="width: 50px;">' + sheetOption.data.colHead[i - 1].myear.replace(" 20", ",") + '</th>';
                    year = sheetOption.data.colHead[i - 1].myear;
                    //i++;
                }
                else if (year != sheetOption.data.colHead[i - 1].myear) {
                    ...
                    curColHead = '<th style="width: 50px;"></th>';
                }
                colHeadHtml += curColHead;
            }
            colHeadHtml += '</tr>';
            ...
            for (var i = 0, curColHead = ''; i <= sheetOption.data.dimensions[1]; ++i) {
                if (i === 0) {
                    //curColHead = '<th class="TimeSheet-head"><a onclick="GetRowsData();">' + sheetOption.data.sheetHead.name + '</a></th>';
                    ...
                    curColHead = '<th title="' + (sheetOption.data.colHead[i - 1].title ? sheetOption.data.colHead[i - 1].title : "") + '" data-col="' + (i - 1) + '" class="TimeSheet-colHead ' + (i === sheetOption.data.dimensions[1] ? 'rightMost' : '') + '" >' + sheetOption.data.colHead[i - 1].name + '</th>';
                }
                colHeadHtml += curColHead;
            }
            colHeadHtml += '</tr>';
        ...
        var initRows = function () {
            for (var row = 0, curRowHtml = ''; row < sheetOption.data.dimensions[0]; ++row) {
                if (sheetOption.data.rowHead[row].type == 1) {
                    roomheader = true;
                    curRowHtml = '<tr class="TimeSheet-row parent">'
                }
                else {
                    roomheader = false;
                ...
                for (var col = 0, curCell = ''; col <= sheetOption.data.dimensions[1]; ++col) {
                    if (col === 0) {
                        if (roomheader) {
                            curCell = '<td title="' + (sheetOption.data.rowHead[row].title ? sheetOption.data.rowHead[row].title : "") + '"class="roomhighlight text-left TimeSheet-rowHead ' + (row === sheetOption.data.dimensions[0] - 1 ? 'bottomMost ' : ' ') + '" style="' + (sheetOption.data.rowHead[row].style ? sheetOption.data.rowHead[row].style : '') + '"><input type="checkbox" class="hddesc" ctype="R" rid="' + sheetOption.data.rowHead[row].irid + '" pid="' + sheetOption.data.rowHead[row].irpid + '" style="margin-right: 7px;"/><a class="blue" style="cursor:pointer; font-weight:Bold;" onclick="Toggletr(this);">' + sheetOption.data.rowHead[row].name + '</a><span class="pull-right"><img src="../images/downarrow.png"></span></td>';
                        }
                        else {

                            //check for linked rate plans
                            if (sheetOption.data.rowHead[row].ilinked > 0)
                                curCell = '<td title="' + (sheetOption.data.rowHead[row].title ? sheetOption.data.rowHead[row].title : "") + '"class="text-left TimeSheet-rowHead ' + (row === sheetOption.data.dimensions[0] - 1 ? 'bottomMost ' : ' ') + '" style="' + (sheetOption.data.rowHead[row].style ? sheetOption.data.rowHead[row].style : '') + '"><input type="checkbox" class="hddesc" ctype="P"  rid="' + sheetOption.data.rowHead[row].irid + '" pid="' + sheetOption.data.rowHead[row].irpid + '" style="margin-right: 7px;"/><span class="blue">' + sheetOption.data.rowHead[row].name + '</span>   <span class="pull-right"><img src="../images/link.png"></span></td>';
                            else
                                curCell = '<td title="' + (sheetOption.data.rowHead[row].title ? sheetOption.data.rowHead[row].title : "") + '"class="text-left TimeSheet-rowHead ' + (row === sheetOption.data.dimensions[0] - 1 ? 'bottomMost ' : ' ') + '" style="' + (sheetOption.data.rowHead[row].style ? sheetOption.data.rowHead[row].style : '') + '"><input type="checkbox" class="hddesc" ctype="P" rid="' + sheetOption.data.rowHead[row].irid + '" pid="' + sheetOption.data.rowHead[row].irpid + '"  style="margin-right: 7px;"/><span class="blue">' + sheetOption.data.rowHead[row].name + '</span></td>';
                        }
                    } else {
                        curCell = '<td class="TimeSheet-cell  ' + sheetOption.data.sheetData[row][col - 1].class + '" data-row="' + row + '" data-col="' + (col - 1) + '" onclick="getdata(this);"   type="' + sheetOption.data.rowHead[row].type + '" cdate="' + sheetOption.data.colHead[col - 1].cdate + '" roomid="' + sheetOption.data.sheetData[row][col - 1].rid + '" planid="' + sheetOption.data.sheetData[row][col - 1].rpid + '" style="cursor:pointer;">' + (sheetOption.data.sheetData[row][col - 1].value == '' ? '' : parseInt(sheetOption.data.sheetData[row][col - 1].value)) + '</td>';
                    }
                    curRowHtml += curCell;
                }
        ...
        var repaintSheet = function () {
            ...
            thisSheet.find(".TimeSheet-row").each(function (row, rowDom) {
                ...
                curRow.find(".TimeSheet-cell").each(function (col, cellDom) {
                    ...
                    //curCell.html((sheetStates[row][col].value == '' ? '' : parseInt(sheetStates[row][col].value)));
                    //}else if(sheetStates[row][col]===0){
                    //    curCell.html("TimeSheet-cell-selected");
                    //}
                });
            });
        };
