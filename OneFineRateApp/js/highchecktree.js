    ...
    jQuery.fn.highCheckTree = function (settings) {
        ...
        function getChildrenHtml(treesdata) {
            ...
            for (i = 0; i < len; i++) {
                ...
                // MANEET --- USE THIS CODE TO ALLOW CHECKBOX CLICK FROM TEXT. BUT THIS WILL PUSH LONG DATA TO NEW LINE, SO AVOID...
                ...
                result += '<li rel="' + node.item.id + '"><div class="arrow ' + arrowClass + '"></div><div class="checkbox ' + checkClass + '"></div>' +
                             node.item.label + '</li>';
            }
        ...
        container.off('click', '.checkbox').on('click', '.checkbox', function () {
            ...
            $('li>.checked').prev('.collapsed').removeClass('collapsed').addClass('expanded');
            $('li:not(.updated)>.checked').prev('.expanded').click();

            setTimeout(
                  function () {
                      $('.checked+ul>li>.checkbox:not(.checked)').click();
                  }, 500);
        ...
        if (!settings.ReadOnly) {
            $('.arrow').addClass("AllowHower");
            $('.checkbox').addClass("AllowHower");
        }
    };
})(jQuery);
